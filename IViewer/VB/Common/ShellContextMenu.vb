...
#Region "GetContextMenuInterfaces()"
		...
		Private Function GetContextMenuInterfaces(ByVal oParentFolder As IShellFolder, ByVal arrPIDLs As IntPtr()) As Boolean
			Dim pUnknownContextMenu As IntPtr = IntPtr.Zero

			Dim nResult As Integer = oParentFolder.GetUIObjectOf(IntPtr.Zero, CUInt(arrPIDLs.Length), arrPIDLs, IID_IContextMenu, IntPtr.Zero, pUnknownContextMenu)

			If S_OK = nResult Then
				_oContextMenu = DirectCast(Marshal.GetTypedObjectForIUnknown(pUnknownContextMenu, GetType(IContextMenu)), IContextMenu)

				Dim pUnknownContextMenu2 As IntPtr = IntPtr.Zero
				If S_OK = Marshal.QueryInterface(pUnknownContextMenu, IID_IContextMenu2, pUnknownContextMenu2) Then
					_oContextMenu2 = DirectCast(Marshal.GetTypedObjectForIUnknown(pUnknownContextMenu2, GetType(IContextMenu2)), IContextMenu2)
				End If
				Dim pUnknownContextMenu3 As IntPtr = IntPtr.Zero
				If S_OK = Marshal.QueryInterface(pUnknownContextMenu, IID_IContextMenu3, pUnknownContextMenu3) Then
					_oContextMenu3 = DirectCast(Marshal.GetTypedObjectForIUnknown(pUnknownContextMenu3, GetType(IContextMenu3)), IContextMenu3)
				End If
...
#Region "InvokeCommand"
		Private Sub InvokeCommand(ByVal oContextMenu As IContextMenu, ByVal nCmd As UInteger, ByVal strFolder As String, ByVal pointInvoke As Point)
			...
			invoke.ptInvoke = New ShPoint(pointInvoke.x, pointInvoke.y)
...
#Region "InvokeContextMenuDefault"
		Private Sub InvokeContextMenuDefault(ByVal arrFI As FileInfo())
			...
			Dim pMenu As IntPtr = IntPtr.Zero
			...
			Try
				...
				If False = GetContextMenuInterfaces(_oParentFolder, _arrPIDLs) Then
					ReleaseAll()
					Return
				End If
...
#Region "ShowContextMenu()"
		...
		Public Sub ShowContextMenu(ByVal handleOwner As IntPtr, ByVal arrFI As FileInfo(), ByVal pointScreen As Point)
			' Release all resources first.
			ReleaseAll()

			Dim pMenu As IntPtr = IntPtr.Zero
			Dim hook As New LocalWindowsHook(HookType.WH_CALLWNDPROC)
			AddHandler hook.HookInvoked, New LocalWindowsHook.HookEventHandler(AddressOf WindowsHookInvoked)
			...
			Try
				...
				Dim nSelected As UInteger = TrackPopupMenuEx(pMenu, TPM.RETURNCMD, pointScreen.x, pointScreen.y, handleOwner, IntPtr.Zero)
...
#Region "WindowsHookInvoked()"
		...
		Private Sub WindowsHookInvoked(ByVal sender As Object, ByVal e As HookEventArgs)
			Dim cwp As CWPSTRUCT = CType(Marshal.PtrToStructure(e.lParam, GetType(CWPSTRUCT)), CWPSTRUCT)
			...
			If _oContextMenu2 IsNot Nothing AndAlso (cwp.message = CInt(WM.INITMENUPOPUP) OrElse cwp.message = CInt(WM.MEASUREITEM) OrElse cwp.message = CInt(WM.DRAWITEM)) Then
				If _oContextMenu2.HandleMenuMsg(CUInt(cwp.message), cwp.wparam, cwp.lparam) = S_OK Then
					Return
				End If
			End If

			If _oContextMenu3 IsNot Nothing AndAlso cwp.message = CInt(WM.MENUCHAR) Then
				If _oContextMenu3.HandleMenuMsg2(CUInt(cwp.message), cwp.wparam, cwp.lparam, IntPtr.Zero) = S_OK Then
					Return
				End If
			End If
